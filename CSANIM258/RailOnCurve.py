...
while dist <= curveLength:
    ...
    mc.rotate(rotx, roty+90, rotx)
    sleeperCurve = random.randrange(-rotAmp, rotAmp)
    mc.rotate(0, sleeperCurve, 0, r=True)
    mc.rotate(rotx, 0, rotx, r=True)
    sleepers.append(curCube)
    dist += distSleepers
sleeperGroup = mc.group(sleepers, n='Sleepers')

leftCurve = mc.offsetCurve(curveName, d=3)
mc.rebuildCurve(leftCurve[0], s=40)
mc.move(0, 0.6, 0)
leftRail = MakeRailPiece(curve=leftCurve[0], width=0.4)
mc.select(leftCurve[0])
mc.move(0, 1.1, 0)
leftRailTop = MakeRailPiece(curve=leftCurve[0], height=0.2, width=0.59)

rightCurve = mc.offsetCurve(curveName, d=-3)
mc.rebuildCurve(rightCurve[0], s=40)
mc.move(0, 0.6, 0)
rightRail = MakeRailPiece(curve=rightCurve[0], width=0.4)
mc.select(rightCurve[0])
mc.move(0, 1.1, 0)
rightRailTop = MakeRailPiece(curve=rightCurve[0], height=0.2, width=0.59)

track = mc.group(leftRail, rightRail, leftRailTop, rightRailTop, sleeperGroup, n='Track')

mc.delete(leftCurve[0], rightCurve[0])
